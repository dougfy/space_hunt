...
| 26 | **Tutorial mode (restartable)** | ⚠️ In progress | Coach marks in `src/game/coach.ts` — 7 steps (BUILD tab → STATION upgrade → skin picker → undock → fly/dock → scan → help) plus a congratulations card offering follow-on tutorials. Rendered by `drawCoachCallout()`; supports UI rects, world-space rings, HTML overlay buttons, and modal overlays. Shown to first-time users and **always to admins**. Progress persisted to profile as `coachStep`. Feedback prompt gated until the tutorial ends. Restartable via **↺ REPLAY TUTORIAL** in the help panel's Next tab. **Remaining:** individually selectable sections, and a real CONTROLS & STEERING sequence behind the congrats card (currently opens the help panel). |
...
| 32 | **Colony ship sequence too complex** | ❌ Open | Colonization flow unclear: build colony ship → send to target → fly there → dock → press COLONIZE. Players don't understand how to send a ship. Needs step-by-step guidance or simplified flow. |
