...
api.post('/stars/reset', requireDev, async (c) => {
  ...
  for (const user of users) {
    try {
      await redis.hDel(`profile:${user}`, ['economy', 'ships', 'stats', 'discoveredStars', 'lastPosition', 'journeyDone', 'coachStep', 'coachSkipped', 'scannedBodies']);
    } catch { /* ignore */ }
  }
...
api.post('/admin/reset-all', requireDev, async (c) => {
  ...
  for (const user of users) {
    try {
      ...
      await redis.hDel(`profile:${user}`, ['economy', 'ships', 'stats', 'discoveredStars', 'enhancedProbeStars', 'lastPosition', 'journeyDone', 'coachStep', 'coachSkipped', 'scannedBodies']);
